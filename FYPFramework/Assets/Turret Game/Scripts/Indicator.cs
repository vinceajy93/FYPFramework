...
public class Indicator : Mode_Control
{
    ...
    void Start()
    {
        ...
        if (game_mode_Single)
        {
            if (this.CompareTag("Bullet_Rest_E"))
            { // There is no nid for indicator for player 1 in single player
    ...
    void Update()
    {
        ...
        if (On_X && canvas_indicator != null)
        {
            ...
            if (!game_mode_Single)
            {
                ...
                { // If Bullet 1, check in camera 2
                    temp_cam = P2Cam;
                }
                else if (this.CompareTag("Bullet_2"))
                { // If Bullet 2, check in camera 1
                    temp_cam = P1Cam;
                }
