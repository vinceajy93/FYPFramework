...
public class Player_Control : MonoBehaviour {
	...
	void Control(float width, float height, Vector3 player_world_size)
	{
		if (overSprite == true) {
			if (Input.GetMouseButtonDown (0)) {
				...
				Debug.Log ("button count: " + button_count);
				if (button_count > 1) {
					Shoot ();
				} 
			...
			// Check if mousepress is on the Player turret
	...
	void Shoot()
	{
		...
		GameObject new_bullet = Instantiate (Resources.Load ("bullet"), new Vector3 (shoot_position.x, shoot_position.y, 0f), shoot_location.transform.rotation) as GameObject; 
