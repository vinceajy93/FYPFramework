...
public class Enemy : MonoBehaviour
{
    ...
    void Shoot()
    {
        ...
        {
            new_bullet = Instantiate(GameObject.FindGameObjectWithTag("Bullet_E"), Vector3.zero, Quaternion.identity) as GameObject;
            new_bullet.transform.SetParent(null);
            new_bullet.transform.position = shoot_position;
            new_bullet.transform.rotation = shoot_location.transform.rotation;
            new_bullet.tag = "Bullet_E";
    ...
    void Move_Bullet_Selective_Bullet(GameObject[] enemy_bullet)
    { // Select which bullet is closer to obstacle / bullet
        ...
        foreach (GameObject AI_Bullet in enemy_bullet)
        { // Check all AI bullet with player's bullet and obstacle and see which is closes to danger
            if (obstacle.Length > 0)
            { //Check if there is obstacle first then loop though the whole list
            ...
            { //Check if there is bullet first then loop though the whole list
    ...
    void Move_Bullet_Aim()
    { // Aim at obstacle / bullet depending on which is closer, else aim at player's turret
        if (!pos_set)
        {
            GameObject[] obstacle = GameObject.FindGameObjectsWithTag("Obstacle");
            GameObject[] player_bullet = GameObject.FindGameObjectsWithTag("Bullet_1");
